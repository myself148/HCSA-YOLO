...
from .metrics import bbox_iou, probiou, bbox_mpdiou, bbox_inner_iou, bbox_focaler_iou, bbox_inner_mpdiou, bbox_focaler_mpdiou, wasserstein_loss, WiseIouLoss, gcd_loss
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16):
        ...
        # GCD
        self.gcd_loss = False
        self.iou_ratio = 0.4 # total_iou_loss = self.iou_ratio * iou_loss + (1 - self.iou_ratio) * (nwd_loss or gcd_loss)
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def __init__(self, model):
        ...
        # 🌟 外挂 1：初始化边界 Loss 并获取检测头引用
        self.baf_loss = DetailAggregateLoss(alpha=0.5)
        self.head = model.model[-1]  # 这会自动拿到 OBB_BAF 实例
    ...
    def __call__(self, preds, batch):
        ...
        # 🌟 外挂 2：计算边界损失并累加
        if hasattr(self.head, 'b_cache') and self.head.b_cache.tensor is not None:
            img = batch["img"] 
            loss_b = self.baf_loss(self.head.b_cache.tensor, img)
            loss[0] += loss_b  
            self.head.b_cache.tensor = None # 阅后即焚，释放显存
...
# ====================================================================
# 🌟 新增 BAFNet: 边界辅助分支损失函数 (DetailAggregateLoss)
# ====================================================================
class DetailAggregateLoss(nn.Module):
    """BAFNet 的拉普拉斯金字塔多尺度边界损失"""
    def __init__(self, alpha=0.5):
        ...
        self.fuse_kernel = nn.Parameter(torch.tensor(
            [[6./10], [3./10], [1./10]], dtype=torch.float32).reshape(1, 3, 1, 1))

    def forward(self, boundary_logits, img):
        if img.shape[1] == 3:
        ...
        b_x1 = F.conv2d(img_gray, self.laplacian_kernel, padding=1).clamp(min=0)
        b_x2 = F.conv2d(img_gray, self.laplacian_kernel, stride=2, padding=1).clamp(min=0)
        b_x4 = F.conv2d(img_gray, self.laplacian_kernel, stride=4, padding=1).clamp(min=0)

        target_shape = b_x1.shape[2:]
        b_x2_up = F.interpolate(b_x2, target_shape, mode='nearest')
        b_x4_up = F.interpolate(b_x4, target_shape, mode='nearest')

        b_x1 = (b_x1 > 0.1).float()
        b_x2_up = (b_x2_up > 0.1).float()
        b_x4_up = (b_x4_up > 0.1).float()

        b_pyramid = torch.stack((b_x1, b_x2_up, b_x4_up), dim=1).squeeze(2)
        b_gt = F.conv2d(b_pyramid, self.fuse_kernel.to(device))
        b_gt = (b_gt > 0.1).float()

        if boundary_logits.shape[2:] != b_gt.shape[2:]:
            boundary_logits = F.interpolate(
                boundary_logits, b_gt.shape[2:], mode='bilinear', align_corners=True)

        pos_weight = torch.tensor([5.0], device=device) 
        loss = F.binary_cross_entropy_with_logits(boundary_logits, b_gt, pos_weight=pos_weight)
