...
class SmoothGIoULoss(nn.Module):
    """
    Smooth Generalized Intersection over Union Loss for Oriented Object Detection.
    
    This loss function is specifically designed for OBB tasks in remote sensing images.
    It addresses the boundary discontinuity problem and provides adaptive learning intensity
    based on GIoU values.
    
    Reference: "Smooth GIoU Loss for Oriented Object Detection in Remote Sensing Images"
    """
    
    def __init__(self, k=5.0, beta=0.75):
        """
        Initialize Smooth GIoU Loss.
        
        Args:
            k (float): Scaling factor for the baseline GIoU loss. Default: 5.0
            beta (float): Threshold for switching between GIoU_L1 and GIoU_L2. Default: 0.75
        """
    ...
    def forward(self, pred_bboxes, target_bboxes):
        """
        Compute Smooth GIoU Loss.
        
        Args:
            pred_bboxes (torch.Tensor): Predicted oriented bounding boxes, shape (N, 5), format xywhr
            target_bboxes (torch.Tensor): Target oriented bounding boxes, shape (N, 5), format xywhr
            
        Returns:
            torch.Tensor: Smooth GIoU loss value
        """
        # Calculate GIoU using the existing probiou function
        giou = probiou(pred_bboxes, target_bboxes, CIoU=True)
        
        # Calculate baseline GIoU loss (IGIoU formulation)
        bg_loss = self.k * math.log(2) - self.k * torch.log(1 + giou)
        
        # Apply smooth GIoU loss
        smooth_giou_loss = torch.where(
            giou < self.beta,
            # GIoU_L1: relatively constant gradient for small GIoU values
            bg_loss - 0.5 * bg_loss[giou < self.beta].mean(),
            # GIoU_L2: decreasing gradient for large GIoU values
            0.5 * (bg_loss ** 2) / bg_loss[giou >= self.beta].mean()
        )
        
        return smooth_giou_loss.mean()
# －－－－－－－smooth_giou_loss－－－－－－－－－－－－
...
class RotatedBboxLoss(BboxLoss):
    ...
    def __init__(self, reg_max, use_smooth_giou=True, k=5.0, beta=0.75):
        """Initialize the BboxLoss module with regularization maximum and DFL settings."""
        super().__init__(reg_max)
        # －－－－－－－smooth_giou_loss－－－－－－－－－－－－
        self.use_smooth_giou = use_smooth_giou
        if self.use_smooth_giou:
            self.smooth_giou_loss = SmoothGIoULoss(k=k, beta=beta)
        # －－－－－－－smooth_giou_loss－－－－－－－－－－－－
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        if self.use_smooth_giou:
            # Use Smooth GIoU Loss for better performance on OBB tasks
            loss_iou = self.smooth_giou_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask])
            loss_iou = (loss_iou * weight).sum() / target_scores_sum
        else:
            # Original probiou loss
            iou = probiou(pred_bboxes[fg_mask], target_bboxes[fg_mask])
            loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
